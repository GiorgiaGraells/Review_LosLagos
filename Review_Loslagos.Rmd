...
This is why the last decades aspect as economic limitations and the impact on local stakeholders are been taking into account in order to decrate news PA and not only a selection based in ecological knowledge [@borrini2004indigenous, Brown, 2002?].
...
Sudamerica counts with more relative % of PA , thta any other area int he world.
...
In general terms, number of articles have been increasin during the years, however there are a few years that clearly show a striking decrease in publications (Fig \@ref(fig:Years)). 
...
Adjacents areas to NP comprehended a `r ((DF %>% dplyr::filter(Study.location %in%  c("Adjacent to PNP", "Adjacent to AANP", "Adjacent to VPRNP")) %>% nrow())/nrow(DF)*100)` % of the studies. Most of the articles studied larger areas that comprises Los lagos Region and in particular some of these NP (`r (DF_Location2 %>% dplyr::filter(Study.zone == "Larger areas that comprise NPs") %>% nrow())`). 
...
DF_object <- DF_object %>% 
  ...
  mutate(General_object= str_wrap(General_object, 12))
