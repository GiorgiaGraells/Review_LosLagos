...
Esta situación se ve favorecida pues gran parte de las áreas protegidas se encuentran en lugares remotos y muy distantes de los centros de desarrollo de la sociedad actual (Joppa & Pfaff, 2009) y, por lo tanto, sin mayores vinculaciones con el contexto territorial en que se ubican.
...
In Chile, Los Lagos region is called because...
...
In order identify the elements that allow 
we create a baseline of sytudies in this area we made a revision of scientif literature carry out in these three protected areas and its surroundings in Los lagos Region

that allow a baseline to project scenarios of climate change to systematic conservation planning in PA
...
Among areas of study in this revision, ecosystem functionning and services are the most frecuent with an  ` r ((DF %>% dplyr::filter(`Area of study`=="Ecosystem functioning and services") %>% nrow() )/nrow(DF) )*100` % (Fig \@ref(fig: Areas)). The same area has been studied in the three NP, however its proportion in PNP is notably higher than the other NP.
...
DF_Area <- DF_Area %>% 
  mutate(`Biger Area of study`= case_when(`Area of study` %in% c("Arqueological","Paleoecology" )~"Archaeology and Paleoecology",
                                          ...
                                          `Area of study` %in% "Ecosystem functioning and services" ~ "Ecosystem",
...
p1 <-ggplot(DF_Area) + geom_col(aes(x = `Biger Area of study`, y=n)) +labs(y="Number of articles", x="Area of study")  + theme_classic()+   theme(text=element_text(size=18,  family="Times"))+ theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1)) 

DF_Area2 <- DF_Area %>% dplyr::filter(`Study location` %in% c("PNP", "VPRNP", "AANP")) %>% 
    mutate(`Biger Area of study`= case_when(`Area of study` %in% c("Arqueological","Paleoecology" )~"Archaeology and Paleoecology",
                                          `Area of study` %in% c("Climate","Environmental conditions", "Seismiology" )~ "Climate and environment",
                                          `Area of study` %in% c("Biogeography","Species", "Exotic species" )~ "Species and distribution",
                                          `Area of study` %in% c("Ecosystem functioning and services") ~ "Ecosystem",
                                          `Area of study` %in% c("Freshwater biodiversity") ~ "Freshwater biodiversity",
                                          `Area of study` %in% c("Hydrology") ~ "Hydrology",
                                          `Area of study` %in% c("Social-ecological") ~ "Social-ecology"))

p2 <- ggplot(DF_Area2) + geom_col(aes(x = `Biger Area of study`, y=n)) +labs(y="Number of articles", x="Area of study")  + theme_classic()+   theme(text=element_text(size=18,  family="Times"))+ theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1)) + facet_wrap(~`Study location`, scales="free_x")
