...
Location2 <-ggplot(DF_Location2, aes(x = fct_reorder(Study.zone,n, .desc=TRUE), y = n, group = Study.location)) + geom_col(aes(fill=Study.zone), position="stack", color="black") + geom_text(aes(label =Study.location), size=4, position=position_dodge(0.9), angle=90,vjust=0.5, hjust=-0.04 ) + labs(y="Number of articles", x="Study locations")  + theme_classic()+ ylim(c(0,66))+theme(legend.position = "none")+ scale_fill_brewer(palette = "Dark2") + theme(text=element_text(size=18,  family="Times"), axis.text.x = element_text(angle = 0, vjust = 1, hjust=0.5))
Location2
```
...
Data collected from the field is abundant and represents a ` r ((DF %>% dplyr::filter(Mehodology %in% c("Field survey", "Field collection", "Field experiment")) %>% nrow() )/nrow(DF) )*100` % of the studies. Among fiel collection highlights some studies were there is collection of data through methods as dendrochronology (`r DF %>% dplyr::filter(Methodology =="dendrochronology") %>% nrow()` articles) and electric fishing (`r DF %>% dplyr::filter(Methodology =="Electric fishing") %>% nrow()`).  (Fig \@ref(fig: Areas)).
...
DF_GenObj <- DF_object %>%  group_by(`General object`) %>% sum(`No of articles` = n()) %>% ungroup()  %>%
  mutate(`General object`=fct_reorder(`General object`, -n)) %>% mutate(`General object`= str_wrap(`General object`, 10))### arreglar esto con cantidades correctas

p3 <-ggplot(DF_GenObj, aes(x = fct_reorder(`General object`, n, .desc=TRUE), y = n)) + geom_col(position="dodge", color="green") + labs(y="Number of articles", x="Study object")  + theme_classic()+ ylim(c(0,35))+theme(legend.position = "none") + theme(text=element_text(size=18,  family="Times"), axis.text.x = element_text(angle = 0, vjust = 1, hjust=0.5))
p3
